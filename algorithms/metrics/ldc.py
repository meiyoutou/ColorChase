...
def postprocess_edges(tensor, img_shape=None, arg=None, is_inchannel=False):
    ...
    for i_shape in image_shape:
        ...
        for i in range(tmp.shape[0]):
            tmp_img = tmp[i]
            tmp_img = np.uint8(_image_normalization(tmp_img))
...
def _image_normalization(img, img_min=0, img_max=255, epsilon=1e-12):
    img = np.float32(img)
...
class UpConvBlock(nn.Module):
    ...
    def _make_deconv_layers(self, in_features, up_scale):
        layers = []
        all_pads = [0, 0, 1, 3, 7]
